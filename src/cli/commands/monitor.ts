...
export function monitorCommand(): Command {
  ...
  command
    ...
    .option('-i, --updateInterval <seconds>', 'Intervalo de actualización en segundos', '5')
    .option('-m, --metrics <items>', 'Métricas específicas a monitorear (separadas por comas)')
    .option('-l, --log-level <level>', 'Nivel de log (debug, info, warn, error)', 'info')
    .option('--no-dashboard', 'Deshabilitar dashboard web')
    .option('-e, --export <format>', 'Formato de exportación (json, prometheus)', 'json')
    ...
    .action(async (options: MonitorCommandOptions) => {
      try {
        const config: MonitorConfig = {
          ...
          metrics: options.metrics ? options.metrics.split(',').map(m => m.trim())
                                  : ['cpu', 'memory', 'disk', 'network', 'process'],
          logLevel: options.logLevel,
          dashboard: options.dashboard !== '--no-dashboard',
          export: {
            format: options?.format ? (options.format === 'prometheus' ? 'prometheus' : 'json') : 'json',
        ...
        await startMonitoring(config).then(()=> console.log('Monitoring started...'));
