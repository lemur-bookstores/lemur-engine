...
function validateConfig(config: DeployConfig): void {
  ...
  if (!validEnvironments.includes(config.environment)) {
    throw new Error(
      `Entorno inválido: ${config.environment}. Debe ser uno de: ${validEnvironments.join(", ")}`,
    );
  }
}
...
async function loadDeployConfig(config: DeployConfig): Promise<any> {
  const configPath =
    config.configPath || path.join(process.cwd(), "deploy.config.json");
  try {
    ...
    if (config.configPath) {
      throw new Error(
        `No se pudo cargar el archivo de configuración: ${configPath}`,
      );
    }
...
async function checkPrerequisites(_config: any): Promise<void> {
...
async function createBackup(_config: any): Promise<void> {
...
async function executeDeploymentSteps(_config: any): Promise<void> {
...
async function performRollback(version: string, _config: any): Promise<void> {
