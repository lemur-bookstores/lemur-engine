...
export async function generateService(
  ...
  await fs.writeFile(
    path.join(testBasePath, `${config.name}Service.test.ts`),
    testContent,
  );
...
function generateServiceContent(config: ServiceConfig): string {
  ...
  const interfaces = config.interfaces?.length
    ? ` implements ${config.interfaces.join(", ")}`
    : "";
  const dependencies = config.dependencies || [];

  const constructorParams = dependencies
    .map(
      (dep) => `private ${dep.charAt(0).toLowerCase() + dep.slice(1)}: ${dep}`,
    )
    .join(", ");
...
function generateTestContent(config: ServiceConfig): string {
  ...
  const mockDeps = dependencies
    .map(
      (dep) =>
        `let ${dep.charAt(0).toLowerCase() + dep.slice(1)}: jest.Mocked<${dep}>;`,
    )
    .join("\n  ");

  const initMocks = dependencies
    .map(
      (dep) => `    ${dep.charAt(0).toLowerCase() + dep.slice(1)} = {
