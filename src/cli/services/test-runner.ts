...
function buildJestArguments(config: TestConfig): string[] {
  ...
  if (config.tags?.length) args.push('--testPathPattern', config.tags.join('|'));
...
async function executeJest(args: string[], config: TestConfig): Promise<TestResults> {
  return new Promise((resolve, reject) => {
    ...
    jest.on('close', async (_code) => {
      ...
      try {
        const results = await parseTestResults(config.report.directory, config.report.format);
...
async function parseTestResults(directory: string, format: string): Promise<TestResults> {
  try {
    if (format === 'json') {
...
async function processReports(results: TestResults, config: TestConfig): Promise<void> {
