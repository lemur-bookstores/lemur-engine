...
export function validatePluginName(name: string): void {
  ...
  if (!validNameRegex.test(name)) {
    throw new Error(
      "El nombre del plugin solo puede contener letras minúsculas, números y guiones, y debe comenzar con una letra",
    );
  }
}
...
export function validateServiceName(name: string): void {
  ...
  if (name.length < 3 || name.length > 50) {
    throw new Error(
      "El nombre del servicio debe tener entre 3 y 50 caracteres",
    );
  }

  if (!validNameRegex.test(name)) {
    throw new Error(
      "El nombre del servicio solo puede contener letras, números y guiones bajos, y debe comenzar con una letra",
    );
  }
}
