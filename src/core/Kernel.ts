...
} from "../services/storage/StorageImplementations";
...
export class Kernel {
  ...
  private initializeComponents(): void {
    ...
    this.stateManager.registerState(
  ...
  private setupErrorHandlers(): void {
    ...
    if (errorHandler?.metrics && errorHandler?.metrics?.enabled) {
      const metricsStorage = this.createMetricsStorage(
        errorHandler.metrics.storage || "any",
      );
      this.errorHandler.registerHandler(
        new MetricsErrorHandler(
          metricsStorage /*errorHandler.metrics.aggregationInterval*/,
        ),
      );
    }
  }
  ...
  private setupMonitoring(): void {
    if (this.config.monitoring) {
      const monitoringConfig: MonitoringModuleConfig = {
        enableTokenUsageTracking:
          this.config.monitoring.ai.tokenTracking.enabled,
        enableModelPerformanceTracking:
          this.config.monitoring.modelPerformance.enabled,
        enableContextQualityScoring:
          this.config.monitoring.ai.contextQuality.enabled,
        contextQualityScorerConfig: {
          piiDetectionEnabled:
            this.config.monitoring.ai.contextQuality.piiDetection,
          harmfulContentFilterEnabled:
            this.config.monitoring.ai.contextQuality.harmfulContentDetection,
      ...
      for (const pluginName of initOrder) {
        ...
        if (plugin) {
          try {
            ...
            await this.handlePluginStatusChange(
              plugin,
              oldStatus,
              plugin.status(),
            );
  ...
  async shutdown(): Promise<void> {
    ...
    const shutdownOrder = this.pluginRegistry
  ...
  async publishEvent<T>(
    event: Omit<KernelEvent<T>, "id" | "timestamp">,
  ): Promise<void> {
    await this.eventBus.publish({
      ...event,
      id: crypto.randomUUID(),
  ...
  private async handlePluginStatusChange(
    plugin: Plugin,
    oldStatus: string,
    newStatus: string,
  ): Promise<void> {
