...
export class PluginRepository {
    ...
    async backupPlugins(plugins: Map<string, Plugin>): Promise<void> {
        const backupDir = path.join(this.storageDir, 'backups', new Date().toISOString());
    ...
    private async getPluginConfig(plugin: Plugin): Promise<any> {
        const configPath = path.join(this.storageDir, 'config', `${plugin.metadata.name}.json`);
        ...
        try {
            ...
            console.error(`Error loading config for plugin ${plugin.metadata.name}:`, error);
