...
export class ConfigLoader extends BaseConfigLoader {
  ...
  public async loadConfig(configPath?: string): Promise<KernelConfig> {
    ...
    if (configPath) {
      ...
      if (configFile) {
        ...
        console.warn(
          "kernel.config.json not found, using default configuration",
        );
      }
    }
  ...
  private loadFromEnvironment(config: KernelConfig): KernelConfig {
    ...
    const envMappings = {
      ...
      KERNEL_CIRCUIT_BREAKER_FAILURE_THRESHOLD:
        "circuitBreaker.failureThreshold",
      KERNEL_CIRCUIT_BREAKER_RESET_TIMEOUT: "circuitBreaker.resetTimeout",
      KERNEL_CIRCUIT_BREAKER_HALF_OPEN_SUCCESS:
      ...
      KERNEL_PLUGIN_CONFIG_AUTOLOAD_DIRECTORIES:
        "pluginConfig.autoload.directories",
  ...
  public validateConfig(config: KernelConfig): void {
    ...
    if (config.circuitBreaker) {
      if (config.circuitBreaker.failureThreshold < 0) {
        throw new Error(
          "circuitBreaker.failureThreshold must be between 0 and 1",
        );
      }
      ...
      if (config.circuitBreaker.halfOpenSuccessThreshold < 1) {
        throw new Error(
          "circuitBreaker.halfOpenSuccessThreshold must be at least 1",
        );
      }
    }
    ...
    if (config.pluginConfig?.autoload?.enabled) {
      if (
        !config.pluginConfig.autoload.directories ||
        config.pluginConfig.autoload.directories.length === 0
      ) {
        throw new Error(
          "pluginConfig.autoload.directories cannot be empty when autoload is enabled",
        );
      }
    }
  }
