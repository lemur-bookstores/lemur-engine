...
export class ConfigLoaderMediator {
    ...
    private setupEventHandlers(): void {
        this.eventManager.subscribe(
            ConfigChangeEventType.CONFIG_LOADED,
            {
        ...
        this.eventManager.subscribe(
            ConfigChangeEventType.CONFIG_ERROR,
            {
    ...
    public async loadConfig(loaderId: string, configPath?: string): Promise<KernelConfig> {
    ...
    public registerEventHook(eventType: ConfigChangeEventType, hook: ConfigChangeListener): void {
        this.eventManager.subscribe(eventType, hook);
        this.logger.debug(`Registered event hook for: ${eventType}`);
    }

    private getConfigChanges(oldConfig?: KernelConfig, newConfig?: KernelConfig): string[] {
    ...
    private compareObjects(obj1: any, obj2: any, path: string, changes: string[]): void {
