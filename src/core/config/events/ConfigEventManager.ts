...
export class ConfigEventManager {
  ...
  private batchedEvents: Map<ConfigChangeListener, ConfigChangeEvent[]> =
    new Map();
  ...
  private shouldNotifyListener(
    ...
    if (filter.propertyFilter && event.changes) {
      if (
        !event.changes.some((change) => filter.propertyFilter!.includes(change))
      ) {
        return false;
      }
    }
  ...
  private handleDebounceNotification(
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        console.error(
          `Error in debounced listener for event ${event.type}:`,
          error,
        );
      } finally {
