...
class AIMonitor {
  ...
  trackTokenUsage(
    ...
    console.log(
      `Token usage tracked: ${tokens} tokens, $${cost} for model ${model}`,
    );
  ...
  trackMCPOperations(operation: MCPOperation): void {
    ...
    console.log(
      `MCP operation tracked: ${operation.operationType} - ${operation.success ? "success" : "failed"}`,
    );
  ...
  monitorMCPHealth(): HealthStatus {
    ...
    const recentOperations = this.mcpOperations.filter(
      (op) => op.timestamp >= oneMinuteAgo,
    );
    const successfulOps = recentOperations.filter((op) => op.success);
    const successRate =
      recentOperations.length > 0
        ? successfulOps.length / recentOperations.length
        : 1;

    // Verificar latencia promedio
    const avgLatency =
      recentOperations.length > 0
        ? recentOperations.reduce((sum, op) => sum + op.latency, 0) /
    ...
    const checks = {
      ...
      mcp_connectivity:
        recentOperations.length > 0 || this.mcpOperations.length === 0,
    };
  ...
  setAIAlert(id: string, condition: AICondition, _action: AlertAction): void {
    ...
    console.log(
      `AI Alert configured: ${id} - ${condition.metric} > ${condition.threshold}`,
    );
  }
      ...
      if (condition.metric === "token_usage") {
        if (usage.tokens > condition.threshold) {
          this.triggerAlert(
            alertId,
            condition,
            `Token usage exceeded: ${usage.tokens} > ${condition.threshold}`,
          );
        }
      }
    });
      ...
      if (condition.metric === "model_latency") {
        if (latency.latency > condition.threshold) {
          this.triggerAlert(
            alertId,
            condition,
            `Model latency exceeded: ${latency.latency}ms > ${condition.threshold}ms`,
          );
        }
      }
    });
      ...
      if (condition.metric === "context_quality") {
        if (quality.score < condition.threshold) {
          this.triggerAlert(
            alertId,
            condition,
            `Context quality below threshold: ${quality.score} < ${condition.threshold}`,
          );
        }
      }
    });
  ...
  private triggerAlert(
    ...
    console.log(
      `🚨 ALERT [${condition.severity.toUpperCase()}] ${alertId}: ${message}`,
    );
  ...
  getTokenUsageStats(model?: string, timeWindow?: number): any {
    ...
    return {
      ...
      averageTokensPerOperation:
        data.reduce((sum, usage) => sum + usage.tokens, 0) / data.length,
    };
  }
