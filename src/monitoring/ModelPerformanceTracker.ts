...
class ModelPerformanceTracker {
  ...
  trackModelPerformance(
    ...
    console.log(
      `Performance tracked for ${model}: ${latency}ms, ${tokens} tokens, $${cost}, success: ${success}`,
    );
  ...
  private updateModelMetrics(snapshot: PerformanceSnapshot): void {
    ...
    if (!existing) {
      ...
      const successfulRequests =
        existing.successfulRequests + (snapshot.success ? 1 : 0);
      const failedRequests =
        existing.failedRequests + (snapshot.success ? 0 : 1);
  ...
  getAverageLatency(model: string, timeWindow?: number): number {
    let snapshots = this.performanceHistory.filter(
      (s) => s.model === model && s.success,
    );
  ...
  getTokenEfficiency(model: string, timeWindow?: number): number {
    ...
    const totalLatency =
      snapshots.reduce((sum, s) => sum + s.latency, 0) / 1000; // Convert to seconds
  ...
  getPerformanceTrends(model: string, timeWindow: number = 3600): any {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const bucketEnd = new Date(
        cutoff.getTime() + (i + 1) * bucketSize * 1000,
      );
    ...
    const bucketMetrics = buckets
      .map((bucket) => {
        ...
        return {
          averageLatency:
            successful.length > 0
              ? successful.reduce((sum, s) => sum + s.latency, 0) /
                successful.length
              : 0,
    ...
    return {
      latencyTrend: this.analyzeTrend(
        bucketMetrics.map((m) => m!.averageLatency),
      ),
      successRateTrend: this.analyzeTrend(
        bucketMetrics.map((m) => m!.successRate),
      ),
      tokenEfficiencyTrend: this.analyzeTrend(
        bucketMetrics.map((m) => m!.tokenEfficiency),
      ),
      dataPoints: bucketMetrics.length,
    };
  }

  private calculateBucketTokenEfficiency(
    ...
    const totalTokens = snapshots.reduce((sum, s) => sum + s.tokens, 0);
    const totalLatency =
      snapshots.reduce((sum, s) => sum + s.latency, 0) / 1000;
