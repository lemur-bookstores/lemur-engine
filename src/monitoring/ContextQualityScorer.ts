...
class ContextQualityScorer {
    ...
    constructor(config: {
        ...
        this.harmfulContentFilterEnabled = config.harmfulContentFilterEnabled || true;
        this.biasDetectionEnabled = config.biasDetectionEnabled || false; // Experimental
    }

    // Score the quality of a given context
    scoreContext(context: string, metadata: Record<string, any> = {}): QualityScore {
        let score = 1.0;
        ...
        if (!this.hasClearStructure(context)) {
            ...
            suggestions.push('Use markdown or clear formatting to structure the context');
        }
        ...
        if (metadata.relevanceScore && metadata.relevanceScore < 0.5) {
            ...
            suggestions.push('Ensure the context is directly related to the user\'s query');
        }

        // 4. Safety and compliance checks
        if (this.piiDetectionEnabled && this.containsPII(context)) {
            score -= 0.3;
            reasons.push('Contains potential Personally Identifiable Information (PII)');
            suggestions.push('Sanitize the context to remove PII before processing');
        }
        if (this.harmfulContentFilterEnabled && this.containsHarmfulContent(context)) {
            score -= 0.5;
        ...
        if (metadata.timestamp) {
            const contextAge = (new Date().getTime() - new Date(metadata.timestamp).getTime()) / (1000 * 3600 * 24); // in days
            if (contextAge > 30) {
                score -= 0.1;
    ...
    private textToVector(text: string): number[] {
        ...
        const vocabulary = ['the', 'a', 'is', 'in', 'it', 'of', 'to', 'for', 'and', 'with', 'on', 'that', 'this', 'user', 'context', 'data', 'system'];
