...
class MonitoringModule implements IModule {
    ...
    constructor(config: MonitoringModuleConfig) {
        ...
        this.contextQualityScorer = new ContextQualityScorer(this.config.contextQualityScorerConfig || {});
    }
    ...
    public getService<T>(serviceName: 'AIMonitor' | 'ModelPerformanceTracker' | 'ContextQualityScorer'): T {
        switch (serviceName) {
            case 'AIMonitor':
            ...
            default:
                throw new Error(`Service ${serviceName} not found in MonitoringModule.`);
        }
    }
}
