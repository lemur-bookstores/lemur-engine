...
class DatabaseMemento {
    ...
    createSnapshot(collections: Map<string, InMemoryCollection<any>>, description?: string): string {
...
class DatabaseObserver {
    ...
    notify(event: EventType, collection: string, data?: any): void {
        this.observers.forEach(observer => observer.update(event, collection, data));
    }
}
...
class InMemoryCollection<T extends IEntity> {
    ...
    constructor(
        ...
        initialData.forEach(item => {
            if (item.id) {
                ...
                const id = typeof item.id === 'number' ? item.id : parseInt(item.id.toString());
                if (!isNaN(id)) {
    ...
    directInsert(item: Omit<T, 'id'> | T): T {
        const newItem = 'id' in item && item.id ? item as T : { ...item, id: this.nextId++ } as T;
    ...
    redo(): boolean {
        if (this.currentCommandIndex >= this.commandHistory.length - 1) return false;
        ...
        for (const stage of pipeline) {
            ...
            if (stage.$group) {
                ...
                result = Array.from(grouped.entries()).map(([key, items]) => {
                    ...
                    Object.entries(stage.$group).forEach(([field, operation]: [string, any]) => {
                        if (field === '_id') return;

                        if (operation.$sum) {
                            group[field] = items.reduce((sum: number, item: any) =>
                                sum + (item[operation.$sum] || 0), 0);
                        }
                        if (operation.$count) {
                            group[field] = items.length;
                        }
                        if (operation.$avg) {
                            const values = items.map((item: any) => item[operation.$avg] || 0);
                            group[field] = values.reduce((sum: number, val: number) => sum + val, 0) / values.length;
                        }
                    });
                    return group;
                });
