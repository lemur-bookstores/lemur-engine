...
import { MCPBridgeConfig, MCPBridgeMessage, MCPBridgeResponse } from '../../types/mcp';
...
export class MCPBridge {
    ...
    private pendingRequests: Map<string, (response: MCPBridgeResponse) => void> = new Map();
    ...
    public async call(method: string, params: any): Promise<any> {
        if (!this.pythonProcess) {
            throw new MCPBridgeError('Python process is not running. Call start() first.');
        }
        ...
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                ...
                reject(new MCPBridgeTimeoutError(`Request ${id} timed out after ${this.config.timeout}ms`));
            }, this.config.timeout);

            this.pendingRequests.set(id, (response) => {
                clearTimeout(timeout);
                if (response.error) {
                    reject(new MCPBridgeError(response.error.message, response.error.code, response.error.data));
                } else {
    ...
    private spawnPythonProcess(): void {
        ...
        if (!pythonExecutable || !mcpScriptPath) {
            throw new MCPBridgeError('Python executable or MCP script path not configured.');
        }
