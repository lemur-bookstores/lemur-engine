    ...
    describe('scoreContext', () => {
      ...
      `;
        ...
        test('should detect PII and lower the score', () => {
            const context = 'User email is test@example.com and phone is 555-123-4567.';
            const { score, reasons } = scorer.scoreContext(context);
            expect(score).toBeLessThan(0.7);
            expect(reasons).toContain('Contains potential Personally Identifiable Information (PII)');
        });
        ...
        test('should penalize for lack of structure', () => {
            const context = 'I want sales figures for Q4 2024 for North America all products I need total revenue units sold and profit margin.';
            const { score, reasons } = scorer.scoreContext(context);
            expect(score).toBe(0.85); // Adjusted expectation
            expect(reasons).toContain('Lacks clear structure (e.g., headings, lists)');
        }); test('should consider freshness metadata', () => {
            const oldTimestamp = new Date();
            oldTimestamp.setDate(oldTimestamp.getDate() - 40); // 40 days old
            const context = 'This is a valid context but it is quite old and might be outdated.';
            const { score, reasons } = scorer.scoreContext(context, { timestamp: oldTimestamp.toISOString() });
