...
describe("KernelMediator", () => {
  ...
  describe("sendMessage - direct messaging", () => {
    ...
    it("should send message to specific component", async () => {
      ...
      expect(mockComponent2.handleMessage).toHaveBeenCalledWith(
        message,
        "component1",
      );
    ...
    it("should handle sending to non-existent component", async () => {
      ...
      await expect(
        mediator.sendMessage(message, "component1", "nonexistent"),
    ...
    it("should handle different message types", async () => {
      ...
      expect(mockComponent2.handleMessage).toHaveBeenCalledTimes(
        messages.length,
      );
    });
    ...
    it("should handle async component message handling", async () => {
      ...
      const sendPromise = mediator.sendMessage(
        message,
        "component1",
        "component2",
      );
  ...
  describe("sendMessage - broadcast messaging", () => {
    ...
    it("should broadcast message to all components except sender", async () => {
      ...
      expect(mockComponent3.handleMessage).toHaveBeenCalledWith(
        message,
        "component1",
      );
    });
    ...
    it("should broadcast to all when sender is not registered", async () => {
      ...
      expect(mockComponent1.handleMessage).toHaveBeenCalledWith(
        message,
        "unknown-sender",
      );
      expect(mockComponent2.handleMessage).toHaveBeenCalledWith(
    ...
    it("should handle broadcast with no components registered", async () => {
      ...
      await expect(
        emptyMediator.sendMessage(message, "sender"),
      ).resolves.not.toThrow();
    });
    ...
    it("should handle errors from multiple components during broadcast", async () => {
      ...
      await expect(
        mediator.sendMessage(message, "component1"),
      ).rejects.toThrow();
    });
  ...
  describe("integration scenarios", () => {
    ...
    it("should handle dynamic component registration during messaging", async () => {
      ...
      // Start a broadcast
      const broadcastPromise = mediator.sendMessage(
        { type: "dynamic" },
        "external",
      );
  ...
  describe("edge cases and error handling", () => {
    ...
    it("should handle very large number of components", async () => {
      ...
      components.forEach((component) => {
        expect(
          (component as jest.Mocked<KernelComponent>).handleMessage,
        ).toHaveBeenCalledWith({ type: "mass-broadcast" }, "external");
      });
    });
    ...
    it("should handle component handleMessage that returns non-promise", async () => {
      ...
      await expect(
        mediator.sendMessage({ type: "sync" }, "external", "sync-component"),
      ).resolves.not.toThrow();
    });
  });
});
