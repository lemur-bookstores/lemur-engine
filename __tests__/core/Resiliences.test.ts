...
describe("Bulkhead", () => {
  ...
  describe("Constructor", () => {
    ...
    it("should set default timeout if not provided", () => {
      const bulkheadNoTimeout = new Bulkhead({
        maxConcurrent: 1,
        maxQueued: 1,
      });
  ...
  describe("execute", () => {
    ...
    it("should handle operation timeout", async () => {
      ...
      await expect(timeoutBulkhead.execute(slowOperation)).rejects.toThrow(
        "Operation timed out",
      );
    });
...
describe("RetryHandler", () => {
  ...
  describe("execute", () => {
    ...
    it("should respect global timeout", async () => {
      ...
      await expect(shortTimeoutHandler.execute(operation)).rejects.toThrow(
        "Operation timed out",
      );
    });
    ...
    it("should not retry non-retryable errors", async () => {
      ...
      await expect(retryableHandler.execute(operation)).rejects.toThrow(
        "validation error",
      );
      expect(operation).toHaveBeenCalledTimes(1);
    });
