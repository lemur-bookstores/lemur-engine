...
describe("Bulkhead", () => {
  ...
  describe("execute - queueing behavior", () => {
    ...
    it("should reject when queue is full", async () => {
      ...
      await expect(noTimeoutBulkhead.execute(longOperation)).rejects.toThrow(
        BulkheadRejectedError,
      );
      await expect(noTimeoutBulkhead.execute(longOperation)).rejects.toThrow(
  ...
  describe("timeout handling", () => {
    ...
    it("should timeout queued operation", async () => {
      ...
      const queuedOperation = () =>
        new Promise(
          (resolve) => setTimeout(() => resolve("queued"), 2000), // Longer than timeout
        );
    ...
    it("should not timeout when operation completes before timeout", async () => {
      ...
      const quickOperation = () =>
        new Promise((resolve) => setTimeout(() => resolve("quick"), 500));
    ...
    it("should handle bulkhead without timeout", async () => {
      ...
      const longOperation = () =>
        new Promise((resolve) => setTimeout(() => resolve("long"), 2000));
    ...
    it("should return correct stats during execution", async () => {
      ...
      const operation = (id: number) => () =>
        new Promise<string>((resolve) => {
          ...
          setTimeout(
            () => (resolve(id.toString()), print(id.toString(), "completing")),
            50,
          ); // Increased delay to 50ms to avoid timing issues
        });
  ...
  describe("integration scenarios", () => {
    it("should handle mixed success and failure operations", async () => {
      ...
      console.log(
        "Mixed operations - Initial stats:",
        noTimeoutBulkhead.getStats(),
      );
      ...
      console.log(
        "Mixed operations - After execution:",
        noTimeoutBulkhead.getStats(),
      );
    ...
    it("should handle rapid successive operations", async () => {
      ...
      console.log(
        "Rapid operations - Initial stats:",
        highCapacityBulkhead.getStats(),
      );
      ...
      console.log(
        "Rapid operations - After queueing:",
        highCapacityBulkhead.getStats(),
      );
  ...
  describe("edge cases and error handling", () => {
    ...
    it("should handle zero queue capacity", async () => {
      ...
      await expect(zeroQueueBulkhead.execute(quickOp)).rejects.toThrow(
        BulkheadRejectedError,
      );
    });
    ...
    it("should handle concurrent access to stats", () => {
      ...
      const operation = () =>
        new Promise((resolve) => setTimeout(() => resolve("result"), 30));
