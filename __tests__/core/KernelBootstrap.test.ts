...
describe("KernelBootstrap", () => {
  ...
  beforeEach(() => {
    ...
    mockServiceContainer =
      new ServiceContainer() as jest.Mocked<ServiceContainer>;
    mockEventBus = new EventBus() as jest.Mocked<EventBus>;

    // Create mock state manager instance
    mockStateManager =
      new KernelStateManager() as jest.Mocked<KernelStateManager>;
    ...
    (Kernel as jest.MockedClass<typeof Kernel>).mockImplementation(
      () => mockKernel,
    );
    (
      ServiceContainer as jest.MockedClass<typeof ServiceContainer>
    ).mockImplementation(() => mockServiceContainer);
    (EventBus as jest.MockedClass<typeof EventBus>).mockImplementation(
      () => mockEventBus,
    );
    (
      KernelStateManager as jest.MockedClass<typeof KernelStateManager>
    ).mockImplementation(() => mockStateManager);
    (
      ErrorHandlerService as jest.MockedClass<typeof ErrorHandlerService>
    ).mockImplementation(() => mockErrorHandler);
    (
      PluginRegistry as jest.MockedClass<typeof PluginRegistry>
    ).mockImplementation(() => mockPluginRegistry);

    // Mock service container methods
    mockServiceContainer.register = jest.fn();
    mockServiceContainer.get = jest.fn();
    mockServiceContainer.has = jest
      .fn()
    ...
    mockStateManager.getCurrentStateEnum = jest
      .fn()
      .mockReturnValue(KernelState.INITIALIZING);
    ...
    mockKernel.getServiceContainer = jest
      .fn()
      .mockReturnValue(mockServiceContainer);
  ...
  describe("bootstrap", () => {
    ...
    it("should provide initialization context to steps", async () => {
      ...
      const step: InitializationStep = {
        ...
        execute: jest
          .fn()
          .mockImplementation(async (context: InitializationContext) => {
    ...
    it("should handle plugin loading errors", async () => {
      ...
      await expect(bootstrap.bootstrap()).rejects.toThrow(
        "Kernel bootstrap failed: Plugin load failed",
      );
    });
    ...
    it("should handle state transition errors", async () => {
      ...
      await expect(bootstrap.bootstrap()).rejects.toThrow(
        "Kernel bootstrap failed: State transition failed",
      );
    });
  });
  ...
  describe("setupDefaultInitializationSteps", () => {
    ...
    it("should create steps with proper structure", () => {
      ...
      // Find specific steps
      const coreServicesStep = steps.find(
        (step: InitializationStep) => step.name === "setup-core-services",
      );
      const stateManagementStep = steps.find(
        (step: InitializationStep) =>
          step.name === "initialize-state-management",
      );
      const errorHandlingStep = steps.find(
        (step: InitializationStep) => step.name === "setup-error-handling",
      );
  ...
  describe("integration scenarios", () => {
    ...
    it("should handle partial bootstrap failure gracefully", async () => {
      ...
      mockKernel.initialize = jest
        .fn()
        .mockRejectedValue(new Error("Service registration failed"));

      await expect(bootstrap.bootstrap()).rejects.toThrow(
        "Kernel bootstrap failed: Service registration failed",
      );
    });
  ...
  describe("edge cases and error handling", () => {
    ...
    it("should handle null/undefined options", async () => {
      ...
      await expect(bootstrap.bootstrap(null as any)).rejects.toThrow(
        "Kernel bootstrap failed",
      );
    ...
    it("should handle steps that throw synchronous errors", async () => {
      ...
      await expect(bootstrap.bootstrap()).rejects.toThrow(
        "Kernel bootstrap failed: Synchronous error",
      );
    });
