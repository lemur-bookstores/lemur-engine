...
describe('EventBus (Core)', () => {
    ...
    describe('subscribe and publish', () => {
        it('debería publicar un evento y notificar a los suscriptores correctos', async () => {
            ...
            const event: KernelEvent = { id: '1', type: eventType, payload: { data: 'test' }, timestamp: new Date(), source: 'test' };
        ...
        it('debería ejecutar todos los handlers suscritos a un evento en paralelo', async () => {
            ...
            const event: KernelEvent = { id: '2', type: eventType, payload: { userId: 123 }, timestamp: new Date(), source: 'test' };
            eventBus.subscribe(eventType, mockHandler1);
        ...
        it('debería usar canHandle para filtrar a qué handlers notificar', async () => {
            ...
            const event: KernelEvent = { id: '3', type: eventType, payload: {}, timestamp: new Date(), source: 'test' };
        ...
        it('no debería detenerse si un handler lanza un error y debería loguearlo', async () => {
            ...
            const event: KernelEvent = { id: '4', type: eventType, payload: {}, timestamp: new Date(), source: 'test' };
            const erroringHandler = createMockHandler();
            (erroringHandler.handle as jest.Mock).mockRejectedValue(new Error('Handler failed!'));
    ...
    describe('Event Sourcing (getEventHistory)', () => {
        it('debería almacenar todos los eventos publicados en el eventStore', async () => {
            // Arrange
            const event1: KernelEvent = { id: 'ev1', type: 'event.one', payload: {}, timestamp: new Date(), source: 'test' };
            const event2: KernelEvent = { id: 'ev2', type: 'event.two', payload: {}, timestamp: new Date(), source: 'test' };
        ...
        it('debería permitir filtrar el historial de eventos por tipo', async () => {
            ...
            const event1: KernelEvent = { id: 'ev1', type: 'event.one', payload: {}, timestamp: new Date(), source: 'test' };
            const event2: KernelEvent = { id: 'ev2', type: 'event.two', payload: {}, timestamp: new Date(), source: 'test' };
            const event3: KernelEvent = { id: 'ev3', type: 'event.one', payload: {}, timestamp: new Date(), source: 'test' };
