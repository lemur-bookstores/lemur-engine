...
describe('Monitoring Integration Tests', () => {
    ...
    describe('Kernel Monitoring Integration', () => {
        it('should initialize kernel with monitoring module enabled', async () => {
            ...
            const monitoringService = kernel.getServiceContainer().resolve<MonitoringModule>('monitoring');
    ...
    describe('Monitoring Configuration Validation', () => {
        it('should handle missing monitoring configuration gracefully', async () => {
            ...
            expect(() => kernel.getServiceContainer().resolve('monitoring')).toThrow();
        });
        ...
        it('should use default monitoring configuration when partial config provided', async () => {
            ...
            testConfig.monitoring = {
                ai: {
                    enabled: true,
                    tokenTracking: { enabled: true, alertThresholds: { costPerHour: 1, tokensPerMinute: 100 } },
                    latencyTracking: { enabled: true, alertThresholds: { averageLatency: 1000, p95Latency: 2000 } },
                    contextQuality: { enabled: true, minimumScore: 0.7, piiDetection: true, harmfulContentDetection: true },
                    mcpOperations: { enabled: true, alertThresholds: { errorRate: 0.1, responseTime: 1000 } }
                },
                modelPerformance: {
                    enabled: true,
                    trackingWindow: 24,
                    alertThresholds: { successRate: 0.95, averageLatency: 1000, costEfficiency: 0.8 }
                },
